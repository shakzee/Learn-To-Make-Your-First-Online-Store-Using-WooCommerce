<?php
/**
 * Baltic Media Grabber class.
 *
 * @link https://github.com/justintadlock/hybrid-core/blob/master/inc/class-media-grabber.php
 * @package Baltic
 */

/**
 * Baltic Media Grabber.
 *
 * @since  1.0.0
 */
class Baltic_Media {
	...
	public function __construct( $args = array() ) {
		...
		add_filter( 'baltic_media_grabber_embed_shortcode_media', array( $wp_embed, 'run_shortcode' ) );
		add_filter( 'baltic_media_grabber_autoembed_media',       array( $wp_embed, 'autoembed'     ) );
		...
		$this->args    = apply_filters( 'baltic_media_grabber_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_media() {
		return apply_filters( 'baltic_media_grabber_media', $this->media, $this );
	}
	...
	public function do_embed_shortcode_media( $shortcode ) {
		...
		$this->media = apply_filters(
			'baltic_media_grabber_embed_shortcode_media',
			$this->original_media
		);
	}
	...
	public function do_autoembed_media() {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $value ) {
				...
				$embed = trim( apply_filters( 'baltic_media_grabber_autoembed_media', $value[0] ) );
	...
	public function filter_dimensions( $html ) {
		...
		list( $width, $height ) = apply_filters(
			'baltic_media_grabber_dimensions',
